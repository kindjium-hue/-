...
`@default`를 그 자리에 넣어 같은 화면을 흉내 낸다. 게시판을 읽는 위젯이므로
미리보기는 같은 서버에서 열어야 한다(`python3 -m http.server`).
...
def main() -> int:
    path = build()
    print(f"만들었습니다: {path}")
    print("게시판을 읽는 위젯이라 파일을 그냥 열면 목록을 못 읽습니다.")
    print(f"  cd {BASE_DIR} && python3 -m http.server 8020")
    print("  → http://localhost:8020/widget-preview.html")
