...
/* 글쓰기 화면의 제목·본문 칸을 찾을 때 써 볼 후보들 (스킨마다 이름이 다르다) */
const TITLE_KEYS = ["subject", "title", "wr_subject", "post_title"];
const BODY_KEYS = ["content", "contents", "wr_content", "post_content", "memo"];
...
/** 글쓰기 화면에서 제목·본문 칸을 찾아 채운다. 채운 칸을 알려 준다. */
const fillInto = (doc, title, body) => {
  const done = { title: false, body: false };

  const inputs = doc.querySelectorAll("input[type=text], input:not([type])");
  for (let i = 0; i < inputs.length && !done.title; i += 1) {
    const name = `${inputs[i].getAttribute("name") || ""} ${inputs[i].getAttribute("id") || ""}`.toLowerCase();
    for (let k = 0; k < TITLE_KEYS.length; k += 1) {
      if (name.indexOf(TITLE_KEYS[k]) >= 0) {
  ...
  for (let i = 0; i < areas.length && !done.body; i += 1) {
    const name = `${areas[i].getAttribute("name") || ""} ${areas[i].getAttribute("id") || ""}`.toLowerCase();
    let hit = name === "";
    for (let k = 0; k < BODY_KEYS.length; k += 1) {
      if (name.indexOf(BODY_KEYS[k]) >= 0) hit = true;
    }
    if (!hit) continue;
    areas[i].value = body;
    areas[i].dispatchEvent(new Event("input", { bubbles: true }));
    areas[i].dispatchEvent(new Event("change", { bubbles: true }));
    done.body = true;
  }

  // 아임웹 글쓰기 본문은 편집기(contenteditable)나 iframe인 경우가 많다
  const lines = body.split("\n");
  if (!done.body) {
    const boxes = doc.querySelectorAll("[contenteditable=true], [contenteditable=\"\"]");
    for (let i = 0; i < boxes.length && !done.body; i += 1) {
      boxes[i].textContent = "";
      for (let n = 0; n < lines.length; n += 1) {
        const line = doc.createElement("div");
        line.textContent = lines[n];
        boxes[i].appendChild(line);
      }
  ...
  if (!done.body) {
    ...
    for (let i = 0; i < frames.length && !done.body; i += 1) {
      let inner = null;
      try { inner = frames[i].contentDocument; } catch (error) { inner = null; }
      if (!inner || !inner.body) continue;
      inner.body.textContent = "";
      for (let n = 0; n < lines.length; n += 1) {
        const line = inner.createElement("div");
        line.textContent = lines[n];
        inner.body.appendChild(line);
      }
      done.body = true;
    }
  }
...
/** 글쓰기 창을 열고 제목·본문을 자동으로 채운다 */
const openAndFill = () => {
  const title = pick("out-title").value;
  const body = pick("out-body").value;
  const write = setting("write") !== "" ? setting("write") : setting("board");
  const win = window.open(write, "_blank");
  if (!win) {
    told("브라우저가 새 창을 막았습니다. 팝업을 허용하거나 아래 복사 버튼을 쓰세요.");
    return;
  }
  told("글쓰기 창을 열었습니다. 제목·본문을 채우고 있습니다…");

  let tries = 0;
  const step = () => {
    tries += 1;
    let doc = null;
    try { doc = win.document; } catch (error) { doc = null; }
    if (win.closed) return;
    if (doc && doc.readyState === "complete" && doc.body) {
      const done = fillInto(doc, title, body);
      if (done.title || tries > 8) {
        if (done.title && done.body) {
          told("제목과 본문을 채웠습니다. 그 창에서 BEFORE·AFTER 사진을 첨부하고 저장을 누르세요.");
        } else if (done.title) {
          told("제목은 채웠습니다. 본문은 편집기 형태라 자동 입력이 안 되니 «본문 복사»로 붙여넣어 주세요.");
        } else {
          told("자동 입력이 되지 않는 글쓰기 화면입니다. 제목·본문 복사 버튼으로 붙여넣어 주세요.");
        }
        try { win.focus(); } catch (error) { return; }
        return;
      }
    }
    if (tries > 20) {
      told("글쓰기 창을 열었습니다. 자동 입력이 안 되면 제목·본문 복사 버튼을 쓰세요.");
      return;
    }
    setTimeout(step, 500);
  };
...
const setWriteLink = () => {
  ...
  if (write === "" || !boardOk) {
    ...
    link.textContent = "게시판 주소를 먼저 맞춰 주세요";
    pick("auto").hidden = true;
...
const setup = () => {
  ...
  pick("auto").addEventListener("click", openAndFill);
